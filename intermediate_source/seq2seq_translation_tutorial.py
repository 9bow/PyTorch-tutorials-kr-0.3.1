...
######################################################################
# *많은* 예제 문장이 있고 신속하게 훈련하기를 원하기 때문에 
...
######################################################################
# 나는 당신이 이 모델의 결과를 훈련하고 관찰하는 것을 권장하지만, 
...
# 훈련 데이터에는 모든 크기의 문장이 있기 때문에 이 계층을 실제로 
# 만들고 훈련 시키려면 적용 할 수 있는 최대 문장 길이 (인코더 출력의 입력 길이)를 
# 선택해야합니다. 최대 길이의 문장은 모든 어텐션 가중치를 사용하지만 
...
#   Translation <https://arxiv.org/abs/1508.04025>`__ 를 읽으십시오
#
# 학습
# ========
#
...
# -----------------------
#
# 훈련을 위해서, 각 쌍마다 입력 Tensor (입력 문장의 단어 주소)와 
...
# ==========
#
# 평가는 대부분 훈련과 동일하지만 목표가 없으므로 각 단계마다 디코더의 
# 예측을 되돌려 전달합니다.
