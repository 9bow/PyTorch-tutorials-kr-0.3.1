...
표준 Python 패키지를 이용하여 불러온 후 NumPy 배열로 변환하면 됩니다.
...
이 튜토리얼에서는 CIFAR10 데이터셋을 이용할 텐데요, 여기에는 다음과 같은 분류들이
...
1. CIFAR10의 학습용 / 시험(test)용 데이터셋을 ``torchvision`` 을 이용하여
   불러오고, 정규화(nomarlizing)합니다.
2. 합성곱 신경망(Convolution Neural Network)을 정의합니다.
3. 손실 함수를 정의합니다.
4. 학습용 데이터를 이용하여 신경망을 학습합니다.
5. 시험용 데이터를 이용하여 신경망을 검사합니다.

1. CIFAR10를 불러오고 정규화하기
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

``torchvision`` 을 이용하면 매우 쉽게 CIFAR10 데이터를 불러올 수 있습니다.
"""
import torch
import torchvision
